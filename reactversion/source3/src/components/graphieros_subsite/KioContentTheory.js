...
function KioContentTheory() {
    ...
    useEffect(() => {
        if (!postsState || postsState.length) {
            return;
        }
        ...
        // request({ section: 'posts', method:'POST', data:newPost })
        //     .then(response => {
    ...
    }, [postsState, setPostsState])

    return (
        <div>
            {postsState && postsState.map((el, i) => <p key={i}>{el.text}</p>)}
        </div>
    )
}
